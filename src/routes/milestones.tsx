    ...
    <div className="px-4 sm:px-8 py-8 max-w-4xl mx-auto">
      ...
      {next && next.progress && (
        <div className="rounded-2xl bg-gradient-romantic p-5 mb-6 shadow-soft">
          <p className="text-xs uppercase tracking-widest text-foreground/70 mb-1">Próxima conquista</p>
          <p className="font-display text-xl mb-3">{next.icon} {next.title}</p>
        ...
        {badges.map((b) => (
          <div
            ...
            className={`rounded-2xl p-5 border transition ${
              b.unlocked ? "bg-card border-primary/30 shadow-card" : "bg-muted/40 border-border opacity-70"
            }`}
          >
            ...
            {b.progress && (
              <>
                ...
                <p className="text-[11px] text-muted-foreground mt-1">{b.progress.current}/{b.progress.target}</p>
              </>
            )}
