  ...
  return (
    <div className="px-4 sm:px-8 py-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <StatCard label="Emoção top" value={topEmotion?.emoji ?? "—"} valueClass="text-3xl" sub={topEmotion?.label} />
        <StatCard label="Lugares" value={new Set(memories.map((m) => m.location).filter(Boolean)).size} icon={<MapPin className="h-4 w-4" />} />
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <section className="rounded-2xl bg-card border border-border p-5 shadow-card">
          ...
          {topPlaces.length === 0 ? (
            <p className="text-sm text-muted-foreground">Marque a localização nas memórias para aparecerem aqui.</p>
          ) : (
            <ol className="space-y-2">
              {topPlaces.map(([place, count], i) => (
                <li key={place} className="flex items-center gap-3">
                  <span className="h-7 w-7 rounded-full bg-primary/10 text-primary flex items-center justify-center text-sm font-medium">{i + 1}</span>
...
function StatCard({ label, value, sub, icon, valueClass }: { label: string; value: React.ReactNode; sub?: string; icon?: React.ReactNode; valueClass?: string }) {
  return (
